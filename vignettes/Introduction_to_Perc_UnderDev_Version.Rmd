...
plot.conf.mat(informationGain, ordering = NA, labels = TRUE)
```
...
plot.conf.mat(informationGain2, ordering = NA, labels = TRUE)
```
...
```{r}
plot.conf.mat(DominanceProbability$p.hat, ordering = s.rank[[1]]$ID, labels = TRUE)
```
...
When individual-level win-loss relationship uncertainty is the focus of your question, the function `individualWinProb` computes the mean and standard deviation for individual-level win-loss probability. 
...
=======
#### Notes
##### how to phrase what type of the network (); better way to phrase dominance/ win-loss probability matrix (Maybe use ranking, define it, and give examples "like ranking in primate dominance, ranking in chest, seeding 
