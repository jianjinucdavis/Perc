...
A matrix sometimes is not the easiest data format for further explorations. A long-format representation of the matrix is easier. The function `dyadicLongConverter` convert win-loss uncertainty to long format.  
...
```{r}
# find simRankOrder
s.rank <- sim.rank.order(DominanceProbability$p.hat, num = 5, kmax = 5)
head(s.rank)
```
...
pathKuai <- findIDpaths(confmatrix, "Kuai", 3)
pathKuai
```

** Introduce findAllPaths and countPaths**

You could also find all pathways of length equal to or less than `n` using the function `findAllPaths`. For example,
```{r}
allp.3 <- findAllPaths(confmatrix, 3)
str(allp.3)
```

This function is very convenient in getting all pathways of length `n` or less. In the list `findAllPaths` returned, the first element is a matrix listing direct pathways. The second element is a list of matrices representing indirect pathways. 

In the first element, the value in the first column `row` indicates the position of the start of the direct pathway in the raw matrix. The value in the second column `col` indicates the position of the end of the direct pathway in the raw matrix. The row names of the matrix indicates the corresponding ID.
...
```{r}
head(allp.3[[2]][[1]])
allp.3[[1]][which(rownames(allp.3[[1]]) == "Kale"), ]
